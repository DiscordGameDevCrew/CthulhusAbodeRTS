...
namespace Pathfinding {
	...
	public partial class AIPath : AIBase, IAstarAI {
		...
		public virtual void OnTargetReached () {
            ...
            GetComponentInChildren<Animator>().SetTrigger("Update");
        }
		...
		protected override void OnPathComplete (Path newPath) {
            ...
            //! Sort of hacky workaround
            //TODO: Something better than this...
            //Debug.Log(p.GetTotalLength());
            if (GetComponentInChildren<Animator>().GetBool("IsMoving") == false && p.GetTotalLength() > 0.15f)
            {
                transform.GetComponentInChildren<Animator>().SetBool("IsMoving", true);
                transform.GetComponentInChildren<Animator>().SetTrigger("Update");
            }
        }

        public void SetDest(Vector3 v)
        {
            //destination.Set(v.x, v.y, v.z);
            destination = v;
        }
